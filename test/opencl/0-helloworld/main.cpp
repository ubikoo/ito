 ...
 * a GPU or CPU depending on what is available.
 */
cl_context CreateContext()
{
    ...
    /*
     * Select an OpenCL platform. Here, simply choose the first* available
     * platform. Normally, one would query for all available platforms and
    ...
    /*
     * Create an OpenCL context on the platform.  Attempt to create a GPU-based
     * context, and if that fails, try to create a CPU-based context.
     */
...
cl_command_queue CreateCommandQueue(cl_context context, cl_device_id *device)
{
    ...
    errNum = clGetContextInfo(context,
        CL_CONTEXT_DEVICES, 0, NULL, &deviceBufferSize);
    if (errNum != CL_SUCCESS) {
    ...
    errNum = clGetContextInfo(context,
        CL_CONTEXT_DEVICES, deviceBufferSize, devices, NULL);
    if (errNum != CL_SUCCESS) {
        delete [] devices;
    ...
    for (size_t i = 0; i < deviceBufferSize / sizeof(cl_device_id); ++i) {
        ...
        errNum = clGetDeviceInfo(devices[i],
            CL_DEVICE_TYPE, sizeof(cl_device_type), &param_device_type, NULL);
        if (errNum != CL_SUCCESS) {
            delete [] devices;
            std::cerr << "Failed to get device IDs";
            return NULL;
    ...
    /*
     * Choose the first available device. Normally, use all available devices
     * or choose the highest performance device based on OpenCL device queries
     */
...
int main(int argc, char const *argv[])
{
    ...
    std::vector<float> b(kArraySize, 0.0f);
    {
        ...
        std::cout << "Initialize arrays: " << msec.count() << "\n";
    }
    ...
    for (size_t i = 0; i < 1; i++) {
        ...
        auto toc = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double,std::ratio<1,1000>> msec = toc-tic;
        std::cout << "Elapsed time: " << msec.count() << " msec\n";
    }
    if (errNum != CL_SUCCESS) {
